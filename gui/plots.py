...
class PlotTabsWidget(QFrame):
    ...
    def __init__(
        self, parent: Optional[QWidget] = None, tabs: Optional[List[PlotTabSpec]] = None
    ):
        super().__init__(parent)
        ...
        self.model_theta = {
            "lambda": SharedParam(
                {"scaled": 450000, "unit": "nm", "fmt": 4, "dec": 3}, self
            ),
            "tmin": SharedParam({"scaled": 0, "unit": "deg", "fmt": 2, "dec": 2}, self),
            "tmax": SharedParam(
                {"scaled": 9000, "unit": "deg", "fmt": 2, "dec": 2}, self
            ),
        }

        self.model_lambda = {
            "theta": SharedParam(
                {"scaled": 3000, "unit": "deg", "fmt": 2, "dec": 2}, self
            ),
            "lmin": SharedParam(
                {"scaled": 250000, "unit": "nm", "fmt": 4, "dec": 3}, self
            ),
            "lmax": SharedParam(
                {"scaled": 800000, "unit": "nm", "fmt": 4, "dec": 3}, self
            ),
        }
    ...
    def set_plot_settings(self, plots_settings: Dict[str, Any]) -> None:
        self._plot_settings = (
            dict(plots_settings) if isinstance(plots_settings, dict) else {}
        )
    ...
    def show_cached_or_axes(self) -> None:
        ...
        if kind in self._cache:
            ...
            self.canvas.show_axes(
                x_label=x_label, y_label=y_label, title=title, grid=True
            )
...
class PlotCanvasQt(QFrame):
    ...
    def _make_ticks(self, vmin: float, vmax: float, target_ticks: int):
        ...
        for _ in range(1000):
            if t > end + 0.5 * step:
                break
            if t >= vmin - 1e-12 * abs(vmax - vmin) and t <= vmax + 1e-12 * abs(
                vmax - vmin
            ):
                ticks.append(t)
            t += step
    ...
    def paintEvent(self, e) -> None:
        ...
        if (
            self._nearest_poly_idx is not None
            and 0 <= self._nearest_poly_idx < self._poly.size()
        ):
    ...
    def mouseMoveEvent(self, ev) -> None:
        ...
        if (
            best_poly_i is None
            or best_d2 is None
            or best_d2 > (self._snap_px * self._snap_px)
        ):
            if self._nearest_poly_idx is not None:
                self._nearest_poly_idx = None
                self._nearest_data_idx = None
        ...
        data_i = (
            self._poly_i[best_poly_i]
            if best_poly_i < len(self._poly_i)
            else best_poly_i
        )

        if (best_poly_i != self._nearest_poly_idx) or (
            data_i != self._nearest_data_idx
        ):
            self._nearest_poly_idx = best_poly_i
            self._nearest_data_idx = data_i
            self.nearestPointChanged.emit(
                float(self._x[data_i]), float(self._y[data_i]), int(data_i)
            )
            self.update()
