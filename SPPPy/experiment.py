...
class ExperimentSPR:
    ...
    def __init__(self, polarization="p"):
        ...
        self.ia_asylum = self.incidence_angle = 0
    ...
    def R(
        ...
        if spectral_width == 0:
            if angle_range is not None:  # -------------- R(theta) -------------
                ...
                if is_complex:
                    return [
                        self.R_deg(
                            theta=angle if angle else self.incidence_angle, wl=wl
                        )
                        for wl in wl_range
                    ]
                else:
        ...
        if angle_range is not None:
            ...
            def spectral_function(x):
                return spectral_width / (
                    2 * np.pi * ((self.wavelength - x) ** 2 + spectral_width**2 / 4)
                )

            wavelenghts_list = np.linspace(
            ...
            for i in range(len(wavelenghts_list)):
                ...
                R_collect = R_collect + [
                    np.abs(self.R_deg(theta)) ** 2 for theta in angle_range
    ...
    def T(self, angle_range=None, wl_range=None, angle=None, is_complex=False):
        ...
        if angle_range is not None:
            if is_complex:
                return [self.T_deg(theta) for theta in angle_range]
            else:
                a = []
                for theta in angle_range:
                    ...
                    a.append(
                        np.real(k_zN) / np.real(k_z0) * np.abs(self.T_deg(theta) ** 2)
                    )
                return a
        ...
        # ------- R(lambda) ------------
            ...
            if is_complex:
                return [self.T_deg(wl=wl) for wl in wl_range]
            else:
                ...
                for wl in wl_range:
                    ...
                    a.append(
                        np.real(k_zN) / np.real(k_z0) * np.abs(self.T_deg(wl=wl) ** 2)
                    )
                return a
    ...
    def R_deg(self, theta=None, wl=None):
        M0 = self.Transfer_matrix(
            theta if theta else self.incidence_angle, 2 * np.pi / wl if wl else self.k0
        )
        if M0[0, 0] == 0:
            return 1
        else:
    ...
    def R_vs_thickness(
        ...
        # Precompute fixed stack products around target layer.
        ...
        # Resolve lining for varied layer once.
        if layer_index == 0:
            var_lining = "TOP"
    ...
    def Transfer_matrix(self, theta, k_0):
        ...
        def GradLayerMatrix(theta, n, d, grad_num):
            ...
            for i in range(1, self.gradient_resolution - 1):
                ...
                M = np.array(
                    [
                        [np.exp(-1j * kidx) / t, np.exp(-1j * kidx) * r / t],
            ...
            M = np.array(
                [
                    [np.exp(-1j * ki * dx) / t, np.exp(-1j * ki * dx) * r / t],
        ...
        if self.cache:
            ...
            M0 = M @ self.layers[len(self.layers) - 1].S_matrix(
                k_0, kx0, "BOTTOM", self.polarization
            )
            return M0
        ...
        # РУДИМЕНТ ДЛЯ СВЕРКИ c тем, что сверялось со статьями при создании
            ...
            # All layers for 's'
                ...
                r = [
                    (k_z[i] - k_z[i + 1]) / (k_z[i] + k_z[i + 1])
                    for i in range(0, len(n) - 1)
                ]
                t = [2 * (k_z[i]) / (k_z[i] + k_z[i + 1]) for i in range(0, len(n) - 1)]
                M0 = np.array([[1 / t[0], r[0] / t[0]], [r[0] / t[0], 1 / t[0]]])
                for i in range(1, len(n) - 1):
    ...
    def gradient_profiles(self, dpi=None, name=None, save_2_file=None):
        """Parameters.
    ...
    def pointSPR(self, theta_range=[0, 90], wl_range=None):  ####***
        ...
        if wl_range is None:
            theta_min = minimize_scalar(
                lambda x: np.abs(self.R_deg(theta=x)) ** 2,
                bounds=[theta_range[0], theta_range[-1]],
                method="Bounded",
            )
            ...
            wl_min = minimize_scalar(
                lambda x: np.abs(self.R_deg(wl=x * nm)) ** 2,
                bounds=[wl_range[0] / nm, wl_range[-1] / nm],
                method="Bounded",
            )
        ...
        if self.n[len(self.n) - 1].real < self.n[0].real:
            ...
            for a in range(1, len(self.n) - 1):
                if self.n[a].real < self.n[0].real:
                    TIR_ang = np.arcsin(self.n[a].real / self.n[0].real)
                    break
    ...
    def plot_SPR_curve(self, Rmin_curve, plot_2d=False, view_angle=None, title=None):
        """Plot R(ϴ, λ).
