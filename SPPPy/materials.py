...
def M_matrix(k0, kx0, n_1, n_2, h_1, polarization="p"):
    ...
    return np.array(
        [
            [np.exp(-1j * k_z1 * h_1) / t, np.exp(-1j * k_z1 * h_1) * r / t],
...
class Layer:
    ...
    def S_matrix(
        self, k0: complex, kx0: complex, lining: str, polarization: str
    ) -> np.ndarray:
        """
        ...
        if isinstance(self.n, (float, complex, int)):
            ...
            if lining == "MIDLE":
                ...
                for i in range(0, self.gradient_resolution - 1):
                    M_i = M_i @ M_matrix(
                        ...
                        n_arr[i],
                        n_arr[i + 1],
                        self.thickness / self.gradient_resolution,
                        polarization,
                    )
            ...
            if lining == "MIDLE":
                ...
                M1 = np.array([[1.0, r_in_val], [r_in_val, 1.0]])
                ...
                M2 = np.array(
                    [[phase_pl, phase_pl * r_out_val], [phase_mn * r_out_val, phase_mn]]
                )
                ...
                result = np.array([[1.0, r_in_val], [r_in_val, 1.0]])

            else:
                print(
                    "Wrong type of 'lining' parameter for Anisotropic, S matrix is None"
                )
                result = None
...
class Anisotropic:
    ...
    def __setattr__(self, name: str, val: any) -> None:
        ...
        if name in ("n0", "n1", "anisotropic_angle"):
            if hasattr(self, "n0") and hasattr(self, "n1"):
                ...
                self.ny_2 = (self.n0 * np.cos(main_angle_rad)) ** 2 + (
                    self.n1 * np.sin(main_angle_rad)
                ) ** 2
                self.nz_2 = (self.n0 * np.sin(main_angle_rad)) ** 2 + (
                    self.n1 * np.cos(main_angle_rad)
                ) ** 2
                self.nyz = (
                    (self.n0**2 - self.n1**2)
                    * np.sin(main_angle_rad)
                    * np.cos(main_angle_rad)
                )
    ...
    def p_div_q(self, beta, k0):
        """p/q for rij."""
        return (
            SM.sqrt(self.ny_2 * self.nz_2)
            * self.K()
            / SM.sqrt(self.nz_2 - (beta / k0) ** 2)
        )
...
class DispersionABS(ABC):
    ...
    def CRI(self, wavelength):
        ...
        if not hasattr(self, "_cri_func") or not callable(self._cri_func):
...
class CauchyDispersion(DispersionABS):
    ...
    def state_hash(self) -> int:
        if self._state_hash is None:
            self._state_hash = hash((self.A, self.B, self.C))
        return self._state_hash
...
class LorentzDrudeDispersion(DispersionABS):
    ...
    def state_hash(self):
        ...
        if (
            self._wp._hash is None
            or self._wt._hash is None
            or self._w0._hash is None
            or self._ampl._hash is None
            or self._state_hash is None
        ):
    ...
    def show_CRI_permittivity(self, lambda_range, dpi=None):
        ...
        title = (
            f"Комплексная проницаемость {self.name}"
            if self.name
            else "Комплексная проницаемость"
        )
        plt.title(title)
        plt.legend()
    ...
    def __repr__(self):
        ...
        return (
            f"LorentzDrude({self.name}): wp={wp_str}, wt={wt_str}, "
...
class MaterialDispersion(DispersionABS):
    ...
    def __init__(self, material, base_file=None):
        ...
        try:
            ...
            if _show_SPR_errors:
                print(
                    f"Файл базы '{self.base_file}' не найден! Используется n = 1 + 0j."
                )
        ...
        if self.name not in Refraction_data["Element"].values:
            if _show_SPR_errors:
                if self.name != "Air":
                    print(
                        f"Материал '{self.name}' не найден в базе! Используется n = 1 + 0j (воздух)."
                    )
            self.lambda_min = 200e-9
            self.lambda_max = 1200e-9
            self.n_func = lambda x: 1.0
        ...
        data = Refraction_data[Refraction_data["Element"] == self.name][
            ["Wavelength", "n", "k"]
        ]
        ...
        if len(n_pts) == 0 or len(k_pts) == 0:
            if _show_SPR_errors:
                print(
                    f"Нет данных n или k для материала '{self.name}'! Используется n = 1 + 0j."
                )
            self.lambda_min = 200e-9
            self.lambda_max = 1200e-9
            self.n_func = lambda x: 1.0
        ...
        self.n_func = interpolate.interp1d(
            n_pts[:, 0], n_pts[:, 1], fill_value="extrapolate"
        )
        self.k_func = interpolate.interp1d(
            k_pts[:, 0], k_pts[:, 1], fill_value="extrapolate"
        )
    ...
    def add_material(self, element, material_file, source=""):
        ...
        try:
            ...
            Refraction_data = pd.DataFrame(
                columns=["Element", "Wavelength", "n", "k", "Source"]
            )

        if element in Refraction_data["Element"].values:
            if _show_SPR_errors:
                print(
                    f"Материал '{element}' уже существует! Используйте merge_materials."
                )
            return self
    ...
    def merge_materials(
        self, primary, second, new_name, source="", delete_origin=False
    ):
        """
        ...
        if new_name in Refraction_data["Element"].values and not (
            delete_origin and new_name in [primary, second]
        ):
            if _show_SPR_errors:
        ...
        primary_df = Refraction_data[Refraction_data["Element"] == primary][
            ["Wavelength", "n", "k"]
        ]
        ...
        second_df = Refraction_data[Refraction_data["Element"] == second][
            ["Wavelength", "n", "k"]
        ]
        ...
        if delete_origin:
            Refraction_data = Refraction_data[
                ~Refraction_data["Element"].isin([primary, second])
            ]
...
class CompositeDispersion(DispersionABS):
    ...
    def __init__(self, components, fractions, name=None):
        ...
        for c in self.components:
            if not isinstance(c, DispersionABS):
                raise TypeError(
                    "All components must be instances of DispersionABS or its subclasses."
                )
        ...
        if fr.ndim != 1 or len(fr) != len(self.components):
            raise ValueError(
                "fractions must be 1D and have the same length as components"
            )
        if fr.sum() == 0:
    ...
    def __repr__(self):
        ...
        details = ", ".join(
            [f"{f:.2f}*{n}" for f, n in zip(self.fractions, comp_names)]
        )
        name_str = f" ({self.name})" if self.name else ""
...
class ScaledDispersion(DispersionABS):
    ...
    def __init__(
        self,
        base_material: DispersionABS,
        index_scaling: complex = 1.0,
        name: str = None,
    ):
        if not isinstance(base_material, DispersionABS):
            raise TypeError(
                "base_material must be an instance of DispersionABS or its subclass."
            )
        super().__init__(name=name)
    ...
    def state_hash(self) -> int:
        ...
        if self._state_hash is None:
            ...
            base_hash = (
                self._base_material.state_hash
...
def plot_graph(
    ...
    if (
        ...
        and len(y) > 0
        and isinstance(y[0], (list, np.ndarray, np.generic))
    ):
    ...
    if not is_x_single and len(x_list) != num_plots:
        print(
            f"Внимание: количество массивов x ({len(x_list)}) не совпадает с количеством массивов y ({num_plots}). "
    ...
    if noise is not None:
        ...
        if (
            isinstance(noise, (list, np.ndarray))
            and len(noise) > 0
            and isinstance(noise[0], (list, np.ndarray))
        ):
...
def plot_spectrum(
    wavelength, reflection, labels=None, title="Reflection data", dpi=None, fix_wl=None
):
    """
    ...
    def wl_to_rgb(w_nm):
        ...
        gamma = 0.8
        if w_nm < 440:
            R, G, B = -(w_nm - 440.0) / (440.0 - 380.0), 0.0, 1.0
        elif w_nm < 490:
            R, G, B = 0.0, (w_nm - 440.0) / (490.0 - 440.0), 1.0
        elif w_nm < 510:
            R, G, B = 0.0, 1.0, -(w_nm - 510.0) / (510.0 - 490.0)
        elif w_nm < 580:
            R, G, B = (w_nm - 510.0) / (580.0 - 510.0), 1.0, 0.0
        elif w_nm < 645:
            R, G, B = 1.0, -(w_nm - 645.0) / (645.0 - 580.0), 0.0
        else:
            R, G, B = 1.0, 0.0, 0.0

        factor = (
            0.3 + 0.7 * (w_nm - 380.0) / (420.0 - 380.0)
            if w_nm < 420
            else 0.3 + 0.7 * (750.0 - w_nm) / (750.0 - 700.0)
            if w_nm > 700
            else 1.0
        )
    ...
    if len(wl_list) == 1 and len(refl_list) > 1:
        ...
        raise ValueError(
            f"Cannot pair {len(wl_list)} wavelength arrays with {len(refl_list)} reflection arrays."
        )
    ...
    if labels is None:
        ...
        if len(labels) == 0:
            ...
            raise ValueError(
                f"Number of labels ({len(labels)}) must be 1 or {n_curves}."
            )
    else:
    ...
    for i, (wl, r, lbl, mk) in enumerate(zip(wl_list, refl_list, label_list, markers)):
        ...
        sc = ax.scatter(
            wl_nm, r, c=colors, s=25, marker=mk, edgecolors="none", label=lbl
        )
        if lbl is not None:
            any_label = True
    ...
    if fix_wl is not None:
        tir_list = (
            [fix_wl] if not isinstance(fix_wl, (list, tuple, np.ndarray)) else fix_wl
        )
        for ang in tir_list:
...
def plot_complex_reflection(
    r, ang=None, title="", fix_limits=True, cbar_shrink=0.75, cbar_pad=0.05
):
    """
    ...
    circle = plt.Circle(
        (0, 0), 1, color="gray", fill=False, linestyle="--", alpha=0.7, linewidth=1.5
    )
    ...
    if ang is not None:
        ...
        ax.scatter(
            r.real, r.imag, s=40, color="blue", edgecolor="k", alpha=0.9, zorder=10
        )
        ax.plot(r.real, r.imag, "b-", alpha=0.6, linewidth=2, zorder=5)
...
def profile_analyzer(theta_range, reflection_data):
    ...
    point = xMin
    while (reflection_data[point] < yMin + half_height) & (point > 0):
        point -= 1
    left_border = point
