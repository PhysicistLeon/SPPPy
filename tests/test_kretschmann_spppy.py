...
@pytest.mark.parametrize("d_nm", KRETSCH_THICKNESSES_NM)
def test_kretschmann_spppy(d_nm):
    R_ref = reflectance_three_medium_p(THETA_DEG, EPS1, EPS_AG, EPS_AIR, d_nm, LAMBDA0_NM)
    ...
    exp.add(Layer(n_ag, d_nm * nm, "Ag film"))
    exp.add(Layer(n_air, 0, "Air"))
