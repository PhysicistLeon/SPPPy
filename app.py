...
LayerBuilder = Callable[
    [LayerState], Any
]  # Any -> чтобы не упираться в типизацию SPPPy
...
def build_cauchy(st: LayerState):
    ...
    disp_model = CauchyDispersion(
        A=float(p.get("A", 1.3)), B=float(p.get("B", 0.02)), C=float(p.get("C", 0.001))
    )
...
def build_gradient(st: LayerState):
    ...
    im = (
        np.asarray(prof.get("im", []), dtype=float)
        if isinstance(prof.get("im", None), list)
...
class MainWindow(QMainWindow):
    ...
    def draw_current(self) -> None:
            ...
            if kind == "re_theta":
                try:
                    ...
                    QMessageBox.critical(
                        self, "SPPPy", f"Ошибка Unit.R(angle_range):\n{e}"
                    )
                    return
        ...
        # --- сканирование по длине волны ---
            ...
            if not (
                np.isfinite(lmin) and np.isfinite(lmax) and lmax > lmin and lmin > 0
            ):
            ...
            if kind == "re_lambda":
                try:
                    ...
                    QMessageBox.critical(
                        self, "SPPPy", f"Ошибка Unit.R(wl_range):\n{e}"
                    )
                    return
    ...
    def _on_load_scheme(self) -> None:
        ...
        if not isinstance(data, dict):
            QMessageBox.warning(
                self, "Загрузка", "Некорректный формат файла (ожидался JSON-объект)."
            )
            return
    ...
    def _current_canvas_curve(self):
        ...
        if x.size != y.size:
            raise RuntimeError(
                "Некорректные данные графика: размерности x и y не совпадают."
            )
    ...
    def _on_save_plot_image(self) -> None:
        ...
        if not ok:
            QMessageBox.critical(
                self, "Сохранение", "Не удалось сохранить изображение."
            )
    ...
    def _on_save_plot_data(self) -> None:
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Сохранение", f"Не удалось сохранить данные:\n{e}"
            )
    ...
    def _on_plot_display_settings(self) -> None:
        ...
        def apply_from_dialog(st: dict) -> None:
            ...
            try:
                ...
                QMessageBox.critical(
                    self, "Настройки", f"Не удалось сохранить файл настроек:\n{e}"
                )
                return
